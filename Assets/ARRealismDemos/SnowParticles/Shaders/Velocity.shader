//-----------------------------------------------------------------------
// <copyright file="Velocity.shader" company="Google LLC">
//
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                float4 result = current_velocity;
                ...
                if (result.w <= 0 && _EmitParticles == 1) {
                    ...
                    float horizontal_direction = clamp(n8rand(i.uv), 0, 1);
                    result.xyz = float3(horizontal_direction, 0, 0);
                }

                const float lifetime_time_step = unity_DeltaTime.x * (1.0 / _ParticleLifetime);
                result.w -= lifetime_time_step;
                result.w = clamp(result.w , 0, 1);
                return result;
            }
            ENDCG
        }

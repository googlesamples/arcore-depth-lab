...
public class DepthOfFieldEffectController : MonoBehaviour
{
    ...
    private void UpdateShaderVariables()
    {
        ...
        DepthOfFieldMaterial.SetVector(_touchPositionPropertyName, _screenAnchorPosition);
        Vector2 aspectRatio = new Vector2(Screen.height / Screen.width, 1f);
        DepthOfFieldMaterial.SetVector(_aspectRatioPropertyName, aspectRatio);
        DepthOfFieldMaterial.SetFloat(_aperturePropertyName, _aperture);

        // Updates the rendering mode.
        DepthOfFieldMaterial.SetInt(_renderModePropertyName, (int)_renderMode);
        DepthOfFieldMaterial.SetInt(_greyscalePheripheralPropertyName,
        ...
        DepthOfFieldMaterial.SetFloat(_minDepthPropertyName, minDepthM);
        DepthOfFieldMaterial.SetFloat(_depthRangePropertyName, maxDepthM - minDepthM);
    }
    ...
    private void ProcessTouch()
    {
        ...
        switch (_renderMode)
        {
            case DepthOfFieldRenderMode.FocusOnWorldAnchor:
                _screenAnchorPosition = Camera.main.WorldToScreenPoint(FocusPoint.position);
                float depthMin = DepthOfFieldMaterial.GetFloat(_normalizedDepthMinName);
                float depthMax = DepthOfFieldMaterial.GetFloat(_normalizedDepthMaxName);
